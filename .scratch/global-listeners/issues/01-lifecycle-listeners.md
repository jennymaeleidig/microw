...
- [x] Reference-doc entries for the two statics (stub is fine; the full "Global listeners" section is ticket "04 — consumer docs") and a changelog entry under Unreleased.

## Comments

- 2026-08-31: Implemented. Design delta on the reorder AC: the membership
  _bookkeeping_ (z-index raise, MRU, unobserve accounting) stays at its
  original call sites — moving it would have entangled the focus hand-off
  and the unobserveRoot check with the notification order. Only the _emit
  point_ moved: registry's `register`/`unregister` now do bookkeeping only,
