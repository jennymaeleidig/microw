...
4. **Non-operable controls do not pose as operable** — met. Disabled controls are absent, never rendered-disabled: the min control is skipped when the taskbar is globally disabled (growth-time check in the controls module), stripped globally by `MicroW.configure`'s window loop, and resize handles are omitted entirely when `resizable: false`. No stale `aria-disabled` exists (grep: zero).
